...
def enviar_alerta_telegram(triggers):
    if not triggers:
        return

    bot_token = "8699500661:AAFanighdh7-Bhi0KzdMZeUZ_eWRUgFQW58"
    chat_id = "5328226090"

    mensaje = "🚨 *Nuevos Triggers Listos:*\n\n"
    for t in triggers:
        mensaje += f"[{t['side']}] {t['symbol']} | Score: {t['score']} | RVOL: {t['rvol']}\n"

    url = f"https://api.telegram.org/bot{bot_token}/sendMessage"
    data = {
        "chat_id": chat_id,
        "text": mensaje,
        "parse_mode": "Markdown"  # Esto permite usar negritas o formato si quieres
    }
...
async def scan_once():
    ...
    if triggers:
        enviar_alerta_telegram(triggers)
