...
# Universo
UNIVERSE_SIZE = 250                 # Top N contratos por volumen 24h
MIN_24H_QUOTE_VOLUME = 5_000_000   # Liquidez mínima en USDT
...
def compute_prebreakout_metrics(df_raw: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    # LONG Scoring
    position_score_long = 25 * \
        clamp((price_position - 0.50) / (0.90 - 0.50), 0, 1)
    trend_score_long = 15 * clamp((ret_24h + 2) / 10, 0, 1)
    score_long = compression_score + vol_score + \
        position_score_long + trend_score_long

    # SHORT Scoring (Invertimos la posición y la tendencia)
    position_score_short = 25 * \
        clamp(((1.0 - price_position) - 0.50) / (0.90 - 0.50), 0, 1)
    # Premia retornos negativos
    trend_score_short = 15 * clamp((-ret_24h + 2) / 10, 0, 1)
    score_short = compression_score + vol_score + \
        position_score_short + trend_score_short
    ...
    watch_ready_short = (
        range_pct <= MAX_PRE_RANGE_PCT and
        price_position <= (1.0 - MIN_PRE_PRICE_POSITION) and  # ej: <= 0.35
        vol_pressure >= MIN_PRE_VOL_PRESSURE
    )
    ...
    return {
        ...
        "watch_score_long": round(score_long, 1),
        "watch_score_short": round(score_short, 1),
        "watch_ready_long": watch_ready_long,
        "watch_ready_short": watch_ready_short,
...
def compute_breakout_metrics(df_raw: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    local_low = history['low'].tail(TRIGGER_LOOKBACK).min()
    ...
    # LONG Metrics
    breakout_buffer_pct_long = pct(
        signal_close, local_high) if local_high > 0 else 0.0
    breakout_score_long = 35 * \
        clamp((breakout_buffer_pct_long - 0.0) / 1.5, 0, 1)
    change_score_long = 20 * \
        clamp((candle_change_pct - 0.5) / (3.0 - 0.5), 0, 1)
    close_score_long = 15 * clamp((close_in_candle - 0.5) / (0.95 - 0.5), 0, 1)

    # SHORT Metrics
    breakout_buffer_pct_short = pct(
        local_low, signal_close) if signal_close > 0 else 0.0  # Positivo si cae más bajo
    breakout_score_short = 35 * \
        clamp((breakout_buffer_pct_short - 0.0) / 1.5, 0, 1)
    change_score_short = 20 * \
        clamp((-candle_change_pct - 0.5) /
              (3.0 - 0.5), 0, 1)  # Invertimos el pct
    # Premia cerrar cerca del low
    close_score_short = 15 * \
        clamp(((1.0 - close_in_candle) - 0.5) / (0.95 - 0.5), 0, 1)

    rvol_score = 30 * clamp((rvol - 1.0) / (4.0 - 1.0), 0, 1)

    score_long = breakout_score_long + rvol_score + \
        change_score_long + close_score_long
    score_short = breakout_score_short + rvol_score + \
        change_score_short + close_score_short

    trigger_ready_long = (
        signal_close > local_high and
        breakout_buffer_pct_long >= MIN_BREAKOUT_BUFFER_PCT and
    ...
    trigger_ready_short = (
        signal_close < local_low and
        breakout_buffer_pct_short >= MIN_BREAKOUT_BUFFER_PCT and
        rvol >= MIN_TRIGGER_RVOL and
        candle_change_pct <= -MIN_TRIGGER_CHANGE_PCT and
        close_in_candle <= (1.0 - MIN_CLOSE_IN_CANDLE)  # ej: <= 0.40
    )

    return {
        "trigger_score_long": round(score_long, 1),
        "trigger_score_short": round(score_short, 1),
        "rvol": round(rvol, 2),
        "change_15m_pct": round(candle_change_pct, 2),
        "breakout_buffer_pct_long": round(breakout_buffer_pct_long, 2),
        "breakout_buffer_pct_short": round(breakout_buffer_pct_short, 2),
        "close_in_candle": round(close_in_candle, 2),
        "trigger_ready_long": trigger_ready_long,
        "trigger_ready_short": trigger_ready_short
    }
...
async def analyze_symbol(symbol: str, semaphore: asyncio.Semaphore) -> Optional[Dict[str, Any]]:
    async with semaphore:
        try:
            ...
            # Revisamos si califica para alguna dirección
            is_long = pre['watch_ready_long'] or trig['trigger_ready_long']
            is_short = pre['watch_ready_short'] or trig['trigger_ready_short']

            if not is_long and not is_short:
                return None

            # Determinamos la dirección principal (priorizamos el que tenga trigger)
            direction = "SHORT" if (trig['trigger_ready_short'] or (
                is_short and not is_long)) else "LONG"
            ...
            # Asignamos variables según la dirección
            if direction == "LONG":
                stage = "BREAKOUT" if trig['trigger_ready_long'] else "WATCHLIST"
                watch_sc = pre['watch_score_long']
                trig_sc = trig['trigger_score_long']
                breakout_pct = trig['breakout_buffer_pct_long']
            else:
                stage = "BREAKDOWN" if trig['trigger_ready_short'] else "WATCHLIST"
                watch_sc = pre['watch_score_short']
                trig_sc = trig['trigger_score_short']
                breakout_pct = trig['breakout_buffer_pct_short']

            total_score = round(watch_sc * 0.45 + trig_sc *
                                0.45 + catalyst_bonus + oi_bonus, 1)

            return {
                "Dir": "🟢 LNG" if direction == "LONG" else "🔴 SRT",
                "Stage": stage,
                "Symbol": base_coin,
                "Contract": symbol,
                "Total Score": total_score,
                "Watch Score": watch_sc,
                "Trigger Score": trig_sc,
                "RVOL": trig['rvol'],
...
def print_results(results: List[Dict[str, Any]], scan_seconds: float):
    ...
    print("=" * 140)
    print(
        f"🚀 PRE-BREAKOUT/DOWN SCANNER | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 140)
    ...
    # Separar en dataframes
    trigger_df = df[df["Stage"].isin(["BREAKOUT", "BREAKDOWN"])].copy()
    watch_df = df[df["Stage"] == "WATCHLIST"].copy()

    # Agregué "Dir" y ajusté "Break %" basado en el nuevo diccionario
    show_cols = [
        "Dir", "Stage", "Symbol", "Total Score", "Watch Score", "Trigger Score",
        "RVOL", "%Change 15m", "Break %", "OI Δ%", "72h Range %",
    ...
    if not trigger_df.empty:
        ...
        print("-" * 140)
        print(trigger_df[show_cols].to_string(index=False))

    if not watch_df.empty:
        print("\n👀 PRE-BREAKOUT / BREAKDOWN WATCHLIST")
        print("-" * 140)
